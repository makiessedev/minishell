...
/*   Updated: 2025/04/14 17:59:16 by mmorais          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void storage_word_token(t_token **token_lst, char *str, int index,
  ...
  word[i] = '\0';
  lst_add_back_token(
      token_lst, lst_new_token(word, ft_strdup(word), WORD_TOKEN, NORMAL_MODE));
  return;
}
...
static void storage_separator_token(t_token **token_lst, char *str, int index,
  ...
  lst_add_back_token(token_lst,
                     lst_new_token(separator, NULL, type, NORMAL_MODE));
...
int storage_word_or_separator_token(int *i, int start, t_main *main_data) {
  ...
  if (type) {
    if ((*i) != 0 &&
        get_type_separator_token(main_data->user_input, (*i) - 1) == 0)
    ...
    if (type == APPEND_TOKEN || type == HEREDOC_TOKEN || type == PIPE_TOKEN ||
      ...
      storage_separator_token(&main_data->token, main_data->user_input, (*i),
                              type);
      if (type == APPEND_TOKEN || type == HEREDOC_TOKEN)
        (*i)++;
    }
